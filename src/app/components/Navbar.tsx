...
export default function Navbar() {
    ...
    const scrollToComponent = (selectedHref:any) => (event:any) => {
        event.preventDefault();
        const component = document.querySelector(selectedHref);
        console.log(component);
        if (component) {
            const { top } = component.getBoundingClientRect();
            const offset = window.scrollY + top - window.innerHeight / 3;
            console.log("top: ", top);
    ...
    return (
        <div className="border-b-[1px] border-white absolute w-full left-0 lg:h-[120px] md:h-[80px] h-[40px] flex flex-col justify-center">
                ...
                <div className="hidden lg:flex w-full lg:space-x-[30px] xl:space-x-[100px] text-[22px] DmSans text-center justify-center absolute top-[50%] -translate-y-[50%] z-10">
                    {navigation.map((item) => (
                        <Link key={item.name} href={item.href}>
                            <p onClick={scrollToComponent(item.href)}>{item.name}</p>
                        </Link>
                    ))}
                </div>
            </nav>
            ...
            ${pageEnd ? "opacity-1 scale-100" : "opacity-0 scale-50"}`}>
